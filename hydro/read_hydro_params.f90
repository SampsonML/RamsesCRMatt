subroutine read_hydro_params(nml_ok)
  ...
  ! Local variables
  !--------------------------------------------------
  integer::i,ifixed,nboundary_true=0
  ...
  real(dp)::em_bound
  integer::ivar
  logical::force_mom_cr_advect=.false. ! Do CR advection even if no stars
#else
  integer::idim
#endif
...
#ifdef SOLVERmhd
  namelist/mhd_params/ &
       & conduction,conduction_ion,twotemp,isotrope_cond,isotropic_pressure,slopelim_cond,k_perp,f_spitzer,cr_diffusion &
       & ,M0,r_length,testcase,fix_temp_diff,Dcr,v_alfven,saturation,coupling,epsilon_diff,coupling_out_of_conduction &
       & ,semi_implicit,alfven_diff_coeff,flinj,Tfloor,RelVar,TCRmax,TCRmin, cr_f_taucell &
       & ,streaming_diffusion,streaming_heating,DCRmax,nlength_str,cooling_cr,fudge_streamboost &
       & ,force_mom_cr_advect,mom_streaming_diffusion, mom_streaming_heating, gamma_cr &
       & ,debug_bicg,do_limiter_aniso,slope_limiter_aniso,alpha_limiter,epsilon_restartbicg     &
       & ,fecr, napot, napotsmall, explicit_diff, autoswitch_diff, sts_diff, nmax_explicit, cr_c_fraction &
       & ,variable_cr_vmax, cr_nsubcycle, cr_smallr_decouple &
       & ,shock_finder,cr_shock,cr_shock_mag,cr_shock_mach,smooth_shock,eta_mach,tmin_acc,ndist &
       & ,mach_strong,debug_mach,minmach,fthre_cr,naway_inject,napot,B_ave
#endif
...
! MHD sets enums for riemann solvers, hydro just does a string compare
...
#ifdef CRFLX
   if(force_mom_cr_advect) mom_cr_advect=.true.
#endif
#endif
  ...
  do i=1,nboundary
   ...
   boundary_var(i,iegy)=ek_bound+P_bound(i)/(gamma-1.0d0)     
#endif
#if NCR>0
   do ivar=1,ncrvars
      boundary_var(i,icru+ivar-1)=crmom_bound(i,ivar)
   end do
#endif
end do
  ...
  !-----------------------------------
  inener=-1
  imetal=-1 ; idelay=-1 ; ivirial1=-1 ; ivirial2=-1 ; ixion=-1 ; ichem=-1
#ifdef SOLVERmhd
  ...
  if(nener>0) then
     inener=ifixed+1
     ifixed = ifixed + nener ! Ifixed is the last used variable
  endif
  if(metal) then
     imetal=ifixed+1
     ifixed=ifixed+1
     if(is_oxygen) ifixed=ifixed+1 ! Second metal variable for oxygen
  endif
  if(delayed_cooling) then
     idelay=ifixed+1
     ifixed=ifixed+1
  endif
  if(shock_finder) then
     imach=ifixed+1
     ifixed=ifixed+1
  endif
  if(sf_virial) then
  ...
  if(myid==1.and.hydro.and.(nvar>ndim+2)) then
     ...
     if(shock_finder) write(*,*) '   imach   = ',imach
     if(sf_virial)then
