...
#ifdef SOLVERmhd
  real(dp),dimension(1:nvector,1:nvar+3+ncrvars),save::uug,uum,uud
#else
...
#endif
#if NCR>0
       &sum(err_grad_crmom(1:ncrvars)).gt.-(ncrvars) .and.&
#endif
...
subroutine jeans_length_refine(ind_cell,ok,ncell,ilevel)
  ...
  do i=1,ncell
     ...
     do irad=1,nener
#ifdef SOLVERmhd
        etherm=etherm-uold(indi,iegy+3+irad)
#else
        etherm=etherm-uold(indi,iegy+irad)
#endif
     end do
#endif
     ! the temperature
