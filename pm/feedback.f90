...
#if NCR>0
     if(fecr.gt.0.0) then
         ! Cosmic rays injection!! Injecting only into one group for now
#ifdef CRFLX
         if(mom_cr_advect) then
            unew(indp(j),icru)=unew(indp(j),icru)+ethermal(j)*(1d0+RAD_BOOST)*fecr
         endif
#else
...
subroutine mechanical_feedback_fine(ilevel,icount)
  ...
  integer::npart1,npart2,icpu,idim,ip
...
subroutine mech_fine(ind_grid,ind_pos_cell,np,ilevel,nSN,mSN,pSN,mZSN,nphSN,mchSN)
  ...
  real(dp)::m_cen
  real(dp)::pvar(1:nvarMHD+ncrvars)
...
#ifdef CRFLX
         if(mom_cr_advect) then
            !print*,'CR injection A'
            uold(icell,icru)=uold(icell,icru) + fecr*nSN(i)*E_SNII/scale_erg/vol_loc*f_LEFT
         endif
#else
  ...
  do i=1,np
     ...
     do j=1,nSNnei
        ...
        if(cpu_map(father(igrid)).ne.myid) then ! need mpi
           ...
           if(ilevel>ilevel2)then ! touching level-1 cells
              pvar(1:nvarMHD+ncrvars) = unew(icell,1:nvarMHD+ncrvars)
           else
              pvar(1:nvarMHD+ncrvars) = uold(icell,1:nvarMHD+ncrvars)
           endif 
...
#ifdef CRFLX
               if(mom_cr_advect) then
                  !print*,'CR injection B'
                  pvar(icru) = pvar(icru)+fecr*nSN(i)*E_SNII/scale_erg/dble(nSNnei)/vol_nei
               endif
#else
              do irad=0,nener-1
           ...
           ! update the hydro variable
           if(ilevel>ilevel2)then ! touching level-1 cells
              unew(icell,1:nvarMHD+ncrvars) = pvar(1:nvarMHD+ncrvars)
           else
              uold(icell,1:nvarMHD+ncrvars) = pvar(1:nvarMHD+ncrvars)
           endif 
...
subroutine mech_fine_mpi(ilevel)
  ...
  real(dp)::scale_nH,scale_T2,scale_l,scale_d,scale_t,scale_v
  real(dp)::scale_msun,msun2g=2d33,scale_erg, pvar(1:nvarMHD+ncrvars),etot0
...
#ifdef CRFLX
               if(mom_cr_advect) then
                  !print*,'CR injection C'
                  pvar(icru) = pvar(icru)+fecr*SNrecv(11,i)*E_SNII/scale_erg/dble(nSNnei)/vol_nei
               endif
#else
              do irad=0,nener-1
