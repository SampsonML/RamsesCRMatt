...
subroutine condinit(x,u,dx,nn)
  ...
  tmp(1:nn) = (x(1:nn,1)-boxlen*0.5)**2   ! profile along x-axis
  !tmp(1:nn) = (x(1:nn,2)-boxlen*0.5)**2    ! profile along y-axis
#if NDIM>1
...
#endif
   !u(1:nn,inener) = exp(-40.* tmp(1:nn))
   u(1:nn,icru) = 2. - sqrt(tmp(1:nn))
   !print*,'Initializing Eeeee_CR to ',u(1:nn,inener)
