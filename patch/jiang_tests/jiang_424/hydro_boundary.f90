...
subroutine make_boundary_hydro(ilevel)
  ...
  integer::crType !CR-- Type of CR variable =0,1,2,3
        ...
        if((boundary_type(ibound)/10).eq.0)then
           ...
           do ind=1,twotondim
              ...
              do ivar=1,nvar+3+ncrvars
                 ...
                 !if(ivar>=iCRflx .and.ivar<=iCRflx+ncrvars)switch=gs(mod(ivar-iCRflx,ndim)+1)
                 if(ivar>icrU .and.ivar<icrU+ncrvars) then
                  ! ------------------------------------------------------------
                  ! need to switch CR flux
                  ! crVar= ivar = 1,2,3,4,.. = Np, Fx, Fy, Fz, Np, Fx,..
                  crType = mod(ivar-icrU, ndim+1)
                  if(crType .ne. 0) switch=gs(crType) !0=Ncr, 1=Fx, 2=Fy, 3=Fz
                  ! ------------------------------------------------------------
                 endif
              ...
              do i=1,ngrid
                  !uold(ind_cell(i),5) = uold(ind_cell(i),5) - uold(ind_cell(i),9)
                  uold(ind_cell(i),icru) = 3.
                  !uold(ind_cell(i),5) = uold(ind_cell(i),5) + uold(ind_cell(i),9)
              end do
