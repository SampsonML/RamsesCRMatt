...
! Precision for radiative transfer
  ...
  ! Shock parameters
  logical::shock_finder=.false.  ! Shock finder
  logical::cr_shock=.false.      ! Shock finder and CR acceleration
  logical::cr_shock_mag=.false.  ! CR acceleration efficiency function of upstream mag field orientation
  logical::cr_shock_mach=.false. ! CR acceleration efficiency function of Mach and PCR/Pgas (Kang & Ryu 2013)
  logical::smooth_shock=.false.  ! Smooth Mach number with previous neighbor values
  real(dp)::minmach=1.5          ! Minimum mach number to be tagged as shock
  real(dp)::eta_mach=0.1d0       ! acceleration efficiency parameter
  real(dp)::tmin_acc=-1d10       ! Time after which CRs are accelerated
  real(dp)::mach_strong=0.0d0    ! Extend the upstream/downstream search kernel for strong shocks
  real(dp)::fthre_cr=0.5d0       ! Max fraction of thermal energy into CR
  real(dp)::decr_lost,decr_lost_mpi,decr_lost_all
  real(dp)::decr_gain,decr_gain_mpi,decr_gain_all
  real(dp)::decr2_lost,decr2_lost_mpi,decr2_lost_all
  real(dp)::decr2_gain,decr2_gain_mpi,decr2_gain_all
  logical::debug_mach=.false.
  integer::ndist=4
  integer::naway_inject=4
  ...
  logical::isotropic_pressure=.true. ! Pure-diagonal Eddington tensor in CR flux update
  ...
  logical::mom_streaming_diffusion=.false. ! CR streaming treated as a diffusion term in 2mom CR method
  logical::mom_streaming_heating=.false.   ! CR streaming heating term in 2mom CR method
  ...
  integer::i_mv_fp=-1,    i_mv_pmag=-1,       i_mv_crflx_p=-1
