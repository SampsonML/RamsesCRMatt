...
#else
                    ! Scale temperature to K
                    if(ivar_frame(proj_ind)==iegy)then
                       e = 0.0d0
                       do idim=1,ndim
#endif
...
#else
                    ! Scale velocities to km/s
                    if(ivar_frame(proj_ind)>1.and.ivar_frame(proj_ind)<iegy) uvar = uvar*scale_v/1e5
#endif
...
666                          continue
                          ...
                          if((is_cube .and. (cube_face))                           &
                             ...
                             do kk=1,n_movie_vars
                                ...
                                if(movie_vars(kk).eq.i_mv_temp .or. movie_vars(kk).eq.i_mv_p)then
                                   ok_frame=.true.
                                   e = 0.0d0
#ifdef SOLVERmhd
                                   do idim=1,3
                                   ...
                                   enddo
#endif
                                   uvar = (gamma-1.0)*(uold(ind_cell(i),iegy)-e)
                                   if(movie_vars(kk).eq.i_mv_temp) uvar = uvar/uold(ind_cell(i),1)*scale_T2
...
#endif
#ifdef CRFLX
                                 ! Cosmic ray energy density map
                                 else if(movie_vars(kk).eq.i_mv_crflx_p)then
                                    ok_frame=.true.
                                    uvar = 0d0
                                    do icr=0,ncr-1
                                       uvar = uvar + uold(ind_cell(i),iCRu+icr*(ndim+1)*gamma_cr(icr+1))
                                    end do
#endif
                                endif
#ifdef RT
...
subroutine set_movie_vars()
  ...
  do kk=1,n_movie_vars
     if(movie_vars_txt(kk) .eq. 'dm') then
        ...
        if(i_mv_p .eq. -1) i_mv_p = kk
        movie_vars(kk) = i_mv_p

     else if (movie_vars_txt(kk) .eq. '|v|') then
        if(i_mv_speed .eq. -1) i_mv_speed = kk
     ...
     else if (movie_vars_txt(kk) .eq. 'crflx_P') then
        if(i_mv_crflx_p .eq. -1) i_mv_crflx_p = kk
        movie_vars(kk) = i_mv_crflx_p
