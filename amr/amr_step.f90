recursive subroutine amr_step(ilevel,icount)
  ...
  if(nstar_tot .gt. 0) then
      if(.not.mom_cr_advect .and. fecr.gt.0.0) then ! Turn on CR advection due to newborn stars:
        if(myid==1) write(*,*) '*****************************************'
        if(myid==1) write(*,*) 'moment-CR advection turned on at a=',aexp
        if(myid==1) write(*,*) '*****************************************'
        mom_cr_advect=.true.
      endif
   endif
...
#ifdef CRFLX
     if(mom_cr_advect) call cr_set_unew(ilevel)
#endif
  ...
  !-------------------------------------
  ! Matt's addition for energy_fine call
  !-------------------------------------
  ...
  if((hydro).and.(.not.static_gas))then
     ...
     if(conduction)then
        ...
        itemp=1;call conduction_cg(ilevel,itemp)
  ...
  !-----------------------
  ! Do shock finder/CR acc
  !-----------------------
  if(shock_finder)then

     ! Detect shock
     call timer('shock finder','start')
     if(shock_finder)call shock_fine(ilevel)

#ifdef CRFLX
     if(cr_shock)then
        call make_virtual_fine_dp(ediss_cr(1),ilevel)
  
        call timer('cr shock inject','start')
        call crshock_inject_fine(ilevel)
     endif
#endif
