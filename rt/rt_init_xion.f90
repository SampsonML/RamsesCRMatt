...
!-------------------------------------------------------------------------
  ...
  do ind=1,twotondim
     ...
     do i=1,nleaf
        ...
        T2 = uold(ind_leaf(i),iegy)          ! Energy density (kin+heat)
...
#ifdef SOLVERmhd
        do idim=1,ndim
           emag=emag+0.125d0*(uold(ind_leaf(i),idim+iegy)+uold(ind_leaf(i),idim+nvar))**2
        end do
#endif
...
!-------------------------------------------------------------------------
  ...
  ! Compute pressure from energy density
  T2 = vars(iegy)                         ! Energy dens. (kin+heat) [UU]
...
#ifdef SOLVERmhd
  do idim=1,ndim
     emag=emag+0.125d0*(vars(idim+iegy)+vars(idim+nvar))**2
  end do
#endif
  T2 = (gamma-1.0)*(T2-ekk-err-emag)        !        Gamma is ad. exponent
