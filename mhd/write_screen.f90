subroutine write_screen
  ...
  real(kind=8),dimension(:),allocatable::ss,ss_all
#if NCR>0
  integer::icr
  real(qdp),dimension(:,:),allocatable::pcr_all,pcr
#endif
#if NENER>0
  integer::irad
  ...
  allocate(ss(1:ncell),ss_all(1:ncell))
  ...
  ss=0.0D0; ss_all=0.0D0
#if NCR>0
  allocate(pcr(1:ncell,1:ncr),pcr_all(1:ncell,1:ncr))
  pcr=0.0D0; pcr_all=0.0D0
#endif
#if NENER>0
           ...
           do ind=1,twotondim
              ...
              do i=1,ncache
                 if(son(ind_cell(i))==0)then
                    ...
                    if(shock_finder)ss(icell)=uold(ind_cell(i),imach)/dd(icell)
#if NCR>0
                    do icr=1,ncr
                       pcr(icell,icr)=(gamma_cr(icr)-1.0d0)*uold(ind_cell(i),icru+(ndim+1)*(icr-1))
                    enddo
#endif
#if NENER>0
                    do irad=1,nener
...
#if NCR>0
  call MPI_ALLREDUCE(pcr,pcr_all,ncell*ncr,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,info)
  pcr=pcr_all
#endif
#if NENER>0
...
#if NENER>0
     if(shock_finder)then
        write(*,116)'lev      x           d          u          v           w          Pnt      P          A          B          C          Mach'
     else
...
#else
     if(shock_finder)then
        write(*,116)'lev      x           d          u          v           w          P          A          B          C          Mach'
     else
...
113 format(i3,1x,1pe12.5,1x,11(1pe10.3,1x))
