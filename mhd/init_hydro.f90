subroutine init_hydro
  ...
  allocate(uold(1:ncell,1:nvar+3+ncrvars))
  allocate(unew(1:ncell,1:nvar+3+ncrvars))
  ...
  if(nrestart>0)then
     ...
     if((neq_chem.or.rt).and.nvar2.gt.nvar+3+ncrvars)then ! Not OK to drop variables
        if(myid==1) write(*,*)'File hydro.tmp is not compatible'
...
#else
     if(nvar2.ne.(nvar+3+ncrvars))then
...
#endif
                 do ivar=icrU,icrU+ncrvars-1 ! Cosmic ray flux
                    read(ilun)xx
                    do i=1,ncache
                       uold(ind_grid(i)+iskip,ivar)=xx(i)
                    end do
