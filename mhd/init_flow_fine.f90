...
subroutine init_flow
  use amr_commons
  use hydro_commons, ONLY: nvar, uold, ncrvars
  implicit none
...
subroutine region_condinit(x,q,dx,nn)
  ...
  integer ::nn
  real(dp)::dx
  real(dp),dimension(1:nvector,1:nvar+3+ncrvars)::q
...
!#endif
  ...
  q(1:nn,nvar+2)=0.0d0
  q(1:nn,nvar+3)=0.0d0
  q(1:nn,icrU:icrU+ncrvars-1)=0.0d0
#if NVAR > 8
  do ivar=9,nvar
     q(1:nn,ivar)=0.0d0
  end do
...
#ifdef CRFLX
              do ivar=1,ncrvars
                q(i,icru+ivar-1)=crmom_region(k,ivar)
              end do
#endif
