module hydro_parameters
...
#ifdef CRFLX
  integer,parameter::ncrvars=ncr*(ndim+1) ! number of CR flux vectors
#else
  integer,parameter::ncrvars=0 ! Not using CR flux vectors
#endif
  !integer,parameter::iCRflx = nvar+4  ! Starting index in uold for CR flux variables
  logical::mom_cr_advect=.false.       ! Assume no CR advection
  integer,parameter::iCRu = nvar+4     ! Starting index in uold for CR variables
  real(dp)::cr_c_fraction=1.           ! Reduced light-speed fraction
  real(dp),dimension(1:MAXLEVEL)::cr_vmax=0. ! Reduced light-speed in code units
  real(dp)::cr_vgas_max=0.             ! Max gas speed, used for adaptive cr_vmax
  real(dp)::cr_smallr_decouple=1d4     ! Decouple CRs from gas with rho<smallr*this_value
  logical:: variable_cr_vmax = .false. ! Vary CR_vmax with gas velocity?
  real(dp),dimension(1:512)::gamma_cr=4d0/3d0
  integer:: cr_nsubcycle = 1
  ...
  real(dp)::v_alfven=0.0 ! Temporary for tests
  real(dp)::cr_f_taucell=0.5 ! Stability factor for cell CR optical depth
  ...
  real(dp),dimension(1:MAXBOUND,1:nvar+3+ncrvars)::boundary_var
...
#endif
#if NCR>0
real(dp),dimension(1:MAXBOUND,1:ncrvars)::crmom_bound=0.0
real(dp),dimension(1:MAXREGION,1:ncrvars)::crmom_region=0.0
real(dp),dimension(1:ncrvars)::err_grad_crmom=-1.0
#endif
