...
subroutine cmpdt(uu,gg,dx,dt,ncell)
  ...
  integer::ncell
  real(dp)::dx,dt
  real(dp),dimension(1:nvector,1:nvar+3+ncrvars)::uu
  ...
  if(mom_cr_advect) then
      ...
      do icr = 1,ncr
         do k = 1, ncell
            cr_cs(k)=cr_cs(k)+uu(k,icru+(ndim+1)*(icr-1))
         end do
      end do
      do k = 1, ncell
         cr_cs(k)=gamma_cr(icr)*(gamma_cr(icr)-1.0d0)*cr_cs(k)/uu(k,1)
  ...
  if(ischeme.eq.1)then ! induction
...
#ifdef CRFLX
  ! Store maximum gas speed on level for cr_vmax
  if(variable_cr_vmax) then
      cr_vgas_max = max(cr_vgas_max, maxval(ctot(1:ncell)))
  endif
#endif
...
!#if NENER>0
  integer::irad
!#endif
  real(dp)::ug(1:nvector,1:nvar+3+ncrvars)
  real(dp)::um(1:nvector,1:nvar+3+ncrvars)
  real(dp)::ud(1:nvector,1:nvar+3+ncrvars)
...
#if NCR>0
   do irad = 1,ncr
      if(err_grad_crmom(irad) >= 0.)then
         do k=1,nn
            pg=ug(k,nvar+3+irad); pm=um(k,nvar+3+irad); pd=ud(k,nvar+3+irad)
            error=2.0d0*MAX( &
              & ABS((pd-pm)/(pd+pm+floor_var)), &
              & ABS((pm-pg)/(pm+pg+floor_var)) )
            ok(k) = ok(k) .or. error > err_grad_crmom(irad)
         end do
      end if
   end do
#endif
