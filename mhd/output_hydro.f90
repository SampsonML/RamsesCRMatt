subroutine backup_hydro(filename, filename_desc)
  ...
  integer :: unit_out, unit_info, igroup, dim
...
#endif
              do ivar = nvar+4, nvar+3+ncrvars ! Write right CR flux
                 do i = 1, ncache
                    xdp(i) = uold(ind_grid(i)+iskip, ivar)
                 end do
                 igroup = (ivar-icrU)/(ndim+1)+1                 
                 dim = mod(ivar-icrU, ndim+1)
                 if(dim .eq. 0) then
                  write(field_name, '("CRegy_", i0.2)') igroup
                else
                    write(field_name, '("CRflx_", i0.2, "_", a1)') igroup, dim_keys(dim)
                    !write(field_name, '("CRflx_", a1, "_", i0.2)') dim_keys(dim), igroup
                  endif
                 call generic_dump(field_name, info_var_count, xdp, unit_out, dump_info_flag, unit_info)
              end do
