...
subroutine interpol_hydro(u1,ind1,u2,nn)
  ...
  implicit none
  integer::nn
  real(dp),dimension(1:nvector,0:twondim  ,1:nvar+3+ncrvars)::u1
  integer ,dimension(1:nvector,0:twondim)           ::ind1
  real(dp),dimension(1:nvector,1:twotondim,1:nvar+3+ncrvars)::u2
  !----------------------------------------------------------
...
subroutine copy_from_refined_faces(ind1,u,v,w,nn)
  ...
  ! TVD interpolation from coarse faces
  integer::j,k,l,ind,iskip,imax,jmax,kmax,neul=5
#if NDIM>1
  integer::i
#endif
